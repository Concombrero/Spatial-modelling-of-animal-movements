...
MODULE_PREFIX=script.simulations
ORIGINAL_ARGS=("$@")
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
DEFAULT_OUTPUT_ROOT=$REPO_ROOT/script/simulations/output/figure_runs/$TIMESTAMP
...
Options:
--output-root DIR
    Destination root for this campaign.
    Default: $DEFAULT_OUTPUT_ROOT
--python BIN
    Python executable to use.
    Default: .venv/bin/python when available, otherwise python3.
--game-theory-only
    Skip BasicSimulation and rerun only GameTheory.
--weight-sweep-run-config PATH
    Use a saved payoff_weight_nash_heatmap run_config.json as the shared
    parameter source for all game-theory scripts. The weight sweep also
    reuses its saved weight grid.
--skip-weight-sweep
    Skip the expensive Nash weight sweep.
-h, --help
...
from script.simulations.shared_config import TWO_POPULATION_SIMULATION_CONFIG
...
basic_scripts = [
    "script.simulations.BasicSimulation.activity_const_heatmaps",
    "script.simulations.BasicSimulation.activity_const_spread",
    "script.simulations.BasicSimulation.spread_sleep_pattern_diffusion",
]
game_theory_scripts = [
    "script.simulations.GameTheory.payoff_initial_condition_comparison",
    "script.simulations.GameTheory.payoff_weight_nash_heatmap",
