...
MODULE_PREFIX=script.simulations.GameTheory
...
General options:
--output-dir DIR
    Destination folder for all outputs. Required.
--run-config PATH
    Load payoff-matrix parameters from a saved run_config.json.
--python BIN
    Python executable to use.
    Default: .venv/bin/python when available, otherwise python.
--mean-x-axes LIST
    Comma-separated axes for mean analysis.
    Default: w1,w2,cycle1,cycle2.
--mean-show-variance BOOL
    true/false for variance subplot. Default: false.
--replicator-time-span FLOAT
    Replicator final time. Default: 40.
--replicator-time-steps INT
    Replicator sample count. Default: 800.
--replicator-plot-style STYLE
    line or stacked. Default: line.
--payoff-mode MODE
    overlap, population-integral, or net-growth. Default: overlap.

Payoff-matrix options:
--strategy-codes LIST
    Comma-separated subset of activity codes.
  --t-sunset FLOAT
  --weights W1 W2
  --sight-radius FLOAT
...
if "payoff_config" in payload:
    raise SystemExit(
        ...
        "script.simulations.GameTheory.payoff_weight_nash_heatmap instead."
    )
